...
// Use styled(Box) so sx props passed at usage time take effect
const DrawerBox = styled(Box)(({ theme, open }) => ({
  // NOTE: do not hardcode height: '100%' so mobile sx can override
  whiteSpace: "nowrap",
  flexShrink: 0,
  background: "linear-gradient(180deg, #f3f5ffff 0%, #a4c9ffff 100%)",
  borderRight: "none",
...
export default function EcommercePage() {
  ...
  const isMdUp = useMediaQuery(theme.breakpoints.up("md")); // desktop if true
  const navigate = useNavigate();

  const [open, setOpen] = useState(false); // drawer expanded state (desktop)
  const [tab, setTab] = useState(0);
  const [filteredProducts, setFilteredProducts] = useState([]);
  ...
  // Mobile "More" popper (MenuIcon) state
  ...
  return (
    ...
    >
      ...
      <DrawerBox
        open={open}
        // desktop drawer visible only on md+
        sx={{
      ...
      <Box sx={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        >
          {/* Left: title (click resets to categories) */}
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            {!isMdUp && (
              // mobile: show MenuIcon in topbar, this opens "More" popper (option 3)
              <>
        ...
        {/* Main tab content - give bottom padding on mobile so fixed bottom nav doesn't cover */}
        <Box sx={{ flex: 1, overflowY: "auto", pb: { xs: "76px", md: 0 } }}>
          {tab === 0 && (
            <CategoriesPage
              onCategorySelect={(categoryId) => {
                setSelectedCategory(categoryId);
        ...
        {drawerItems.map((it) => {
          ...
          return (
            ...
            >
              ...
              >
                <Badge badgeContent={it.tab === 4 ? wishlistItems.length : it.tab === 2 ? cartItems.length : 0} color="error">
                  {it.icon}
                </Badge>
              </IconButton>
              <Typography sx={{ fontSize: 11, mt: 0.25 }}>{it.label}</Typography>
            </Box>
          );
        })}
      </Box>
