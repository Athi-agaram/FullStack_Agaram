...
public class OrderRepositoryImpl implements OrderRepository {
    ...
    public List<Map<String, Object>> findNotificationsByUsername(String username) {
        Map<String, Object> perm = findUserPermission(username);
        String role = perm == null ? "USER" : (perm.get("permission_type") == null ? "USER" : perm.get("permission_type").toString().toUpperCase());

        logger.info("Fetching notifications for username: " + username + ", role: " + role);

        // CUSTOMER: All notifications for their orders
        if (role.equals("USER") || role.equalsIgnoreCase("CUSTOMER")) {
            String sql =
                "SELECT n.id, n.order_id, n.sender_username, n.message, n.status, n.created_at, " +
                "o.status AS order_status, o.total_amount, o.user_id, u.username AS customer_username, " +
                "(SELECT COUNT(*) FROM order_items oi WHERE oi.order_id = o.id) AS item_count " +
                ...
                "JOIN users u ON o.user_id = u.id " +
                "WHERE u.username = ? " +
                "ORDER BY o.id DESC, n.created_at ASC";
            return jdbc.queryForList(sql, username);
        }

        // ADMIN: All notifications for all orders
        ...
        // WAREHOUSE: All notifications for orders currently at PLACED status
        if (role.equalsIgnoreCase("WAREHOUSE")) {
            String sql =
                "SELECT n.id, n.order_id, n.sender_username, n.message, n.status, n.created_at, " +
                "o.status AS order_status, o.total_amount, o.user_id, u.username AS customer_username, " +
                "(SELECT COUNT(*) FROM order_items oi WHERE oi.order_id = o.id) AS item_count " +
        ...
        // DISTRIBUTOR: All notifications for orders currently at PROCESSING status
        ...
        // AGENT: All notifications for orders currently at SHIPPED status
        ...
        // COURIER: All notifications for orders currently at OUT_FOR_DELIVERY status
        ...
        logger.warning("Unknown role: " + role + " for username: " + username);
        return jdbc.queryForList("SELECT * FROM notifications WHERE 1=0");
    }

    @Override
    public int insertNotification(int orderId, String senderUsername, String message, String status) {
        // Enhanced duplicate check: look for notifications with same order_id, sender, and status
        // created within the last 60 seconds (increased from 5 seconds)
        String checkSql = 
            "SELECT COUNT(*) FROM notifications " +
            "WHERE order_id = ? AND sender_username = ? AND status = ? " +
            "AND DATEDIFF(SECOND, created_at, CURRENT_TIMESTAMP) < 60";
        
        try {
            Integer count = jdbc.queryForObject(checkSql, Integer.class, orderId, senderUsername, status);
            
            if (count != null && count > 0) {
                logger.warning(String.format(
                    "Duplicate notification detected for orderId=%d, sender=%s, status=%s. Skipping insert.", 
                    orderId, senderUsername, status));
                return 0;
            }
        } catch (Exception e) {
            logger.warning("Error checking for duplicate notifications: " + e.getMessage());
        ...
        try {
            ...
            // If we get a unique constraint violation, it's okay - just log it
            if (e.getMessage() != null && e.getMessage().contains("unique")) {
                logger.warning("Unique constraint prevented duplicate notification - this is expected");
                return 0;
            }
            throw e;
        }
    ...
    public int deleteOldNotificationsExceptLatest(int orderId) {
        String sql =
            "DELETE FROM notifications WHERE order_id = ? " +
            "AND id NOT IN (SELECT TOP 1 id FROM notifications WHERE order_id = ? ORDER BY created_at DESC) " +
            "AND id NOT IN (SELECT id FROM notifications WHERE order_id = ? AND status = 'DELIVERED')";
        return jdbc.update(sql, orderId, orderId, orderId);
    }
    ...
    public String getUsernameById(int userId) {
        try {
            ...
            if (!result.isEmpty() && result.get(0).get("username") != null) {
                return result.get(0).get("username").toString();
            }
        } catch (Exception e) {
            logger.warning("Could not fetch username for userId: " + userId);
        }
        return "Customer";
    }
}